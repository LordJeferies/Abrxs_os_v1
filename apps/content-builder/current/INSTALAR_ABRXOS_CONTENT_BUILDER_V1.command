...
INSTALL_DIR="$HOME/Applications/ABRXOS/ABRXOS_CONTENT_BUILDER_V1"
APP_OUT="$HOME/Desktop/ABRXOS Content Builder.app"
FALLBACK="$HOME/Desktop/ABRXOS Content Builder.command"
LOG_DIR="$HOME/Library/Logs"
LOG_FILE="$LOG_DIR/abrxos_content_builder_v1.log"
...
mkdir -p "$INSTALL_DIR" "$LOG_DIR" "$HOME/ABRXOS_CONTENT_BUILDER_DATA" "$HOME/ABRXOS_CONTENT_BUILDER_EXPORTS"
rsync -a --delete --exclude 'tests' --exclude '__pycache__' "$SCRIPT_DIR/" "$INSTALL_DIR/"
LAUNCHER="$INSTALL_DIR/LAUNCH_ABRXOS_CONTENT_BUILDER.command"
cat > "$LAUNCHER" <<EOF
#!/bin/zsh
mkdir -p "$LOG_DIR"
...
echo "✓ Datos: $HOME/ABRXOS_CONTENT_BUILDER_DATA"
echo "✓ Exports: $HOME/ABRXOS_CONTENT_BUILDER_EXPORTS"
