...
const checklistKeys=['editorial','source','sourceAlignment','microtrim','voRecording','cut','xr','images','broll','cine','motion','sfx','music','captions','cover','copy','qa'];
...
function decorateBuild(){
  ...
  const aside=host.querySelector('aside');const v=validation();if(aside&&v&&!aside.querySelector('.r6-validation-summary')){const box=document.createElement('div');box.className='r6-validation-summary';box.innerHTML=`<b>R6 Preflight</b><small>${v.summary?.criticalCount||0} críticos · ${v.summary?.warningCount||0} warnings · ${v.summary?.alphaReadyCount||0} Alpha ready · ${v.summary?.cutterReadyCount||0} Cutter ready · ${v.summary?.projectionReadyCount||0} Projection ready</small>`;aside.prepend(box)}
}
...
window.ABRXOS_GEOMETRA_R6={version:'3.1.1-r6.1',workflow:R6_WORKFLOW,refreshValidation:refreshR6Validation,progressFor:checklistProgress,validationRow};
