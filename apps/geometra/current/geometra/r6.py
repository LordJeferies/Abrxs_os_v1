...
VALIDATION_SCHEMA = "abrxos.r6-validation.v1.1"
...
CHECKLIST_KEYS = (
    "editorial", "source", "sourceAlignment", "microtrim", "voRecording",
    "cut", "xr", "images", "broll", "cine", "motion", "sfx", "music",
    "captions", "cover", "copy", "qa",
)
...
def _source_alignment_report(piece: dict[str, Any]) -> tuple[dict[str, Any], list[str], list[str]]:
    rows = piece.get("sourceAlignment") if isinstance(piece.get("sourceAlignment"), list) else []
    warnings: list[str] = []
    missing: list[str] = []
    microtrim = 0
    located = 0
    ...
    for index, row in enumerate(rows, 1):
        if not isinstance(row, dict):
            missing.append(f"sourceAlignment[{index}]: object")
            continue
        rid = str(row.get("id") or f"sourceAlignment[{index}]")
        status = str(row.get("status") or row.get("timingStatus") or "").lower()
        if status in {"verified", "user_confirmed", "word_aligned", "cue_based", "cue_verified", "estimated"}:
            located += 1
        else:
            unresolved += 1
        if bool(row.get("microtrim") or row.get("requiresMicrotrim")):
            microtrim += 1
            if not (row.get("startAnchor") or row.get("endAnchor")):
                warnings.append(f"{rid}: microtrim sin anchors")
        if not (row.get("desiredText") or row.get("text")):
            missing.append(f"{rid}: desiredText")
        if not (row.get("parentStart") is not None or row.get("parentStartTc")):
            warnings.append(f"{rid}: parent start faltante")
        if not (row.get("parentEnd") is not None or row.get("parentEndTc")):
            warnings.append(f"{rid}: parent end faltante")
    return {
        "count": len(rows),
        "locatedCount": located,
        "microtrimRequiredCount": microtrim,
        "unresolvedCount": unresolved,
    }, warnings, missing


def _voiceover_report(piece: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    parts = piece.get("parts") if isinstance(piece.get("parts"), list) else []
    vo_parts = [p for p in parts if isinstance(p, dict) and (p.get("kind") == "voiceover" or p.get("sourceMode") == "VO_ADDED" or str(p.get("role") or "").upper() == "VOICEOVER")]
    warnings: list[str] = []
    placement_ready = True
    for part in vo_parts:
        pid = str(part.get("partId") or "VO")
        placement = part.get("placement") if isinstance(part.get("placement"), dict) else {}
        if not (placement.get("after") or placement.get("before")):
            warnings.append(f"{pid}: VO_ADDED placement.after/before faltante")
            placement_ready = False
        if part.get("sourceRangeId") not in (None, ""):
            warnings.append(f"{pid}: VO_ADDED no debe fingir sourceRangeId")
    timeline = piece.get("timeline") if isinstance(piece.get("timeline"), list) else []
    vo_events = [e for e in timeline if isinstance(e, dict) and e.get("track") == "vo"]
    return {"count": len(vo_events) or len(vo_parts), "placementReady": bool(placement_ready)}, warnings


def _projection_report(piece: dict[str, Any]) -> tuple[bool, list[str]]:
    timeline = piece.get("timeline") if isinstance(piece.get("timeline"), list) else []
    images = [e for e in timeline if isinstance(e, dict) and e.get("track") == "images"]
    motions = [e for e in timeline if isinstance(e, dict) and e.get("track") == "motion"]
    sfxs = [e for e in timeline if isinstance(e, dict) and e.get("track") == "sfx"]
    image_refs = {str(e.get("assetId") or (e.get("asset") or {}).get("assetId") or "") for e in images}
    motion_refs = {str(e.get("id") or "") for e in motions}
    sfx_refs = {str(e.get("id") or "") for e in sfxs}
    missing: list[str] = []
    for xr in [e for e in timeline if isinstance(e, dict) and e.get("track") == "xr"]:
        xid = str(xr.get("id") or "XR")
        for asset in xr.get("assets") or []:
            if not isinstance(asset, dict):
                continue
            aid = str(asset.get("assetId") or asset.get("id") or "")
            timed = _num(asset.get("start")) is not None and _num(asset.get("end")) is not None
            if aid and timed and aid not in image_refs:
                missing.append(f"{xid}/{aid}: falta proyección T3 image")
        for mid in xr.get("motionIds") or []:
            if str(mid) not in motion_refs:
                missing.append(f"{xid}/{mid}: falta proyección T4 motion")
        for sid in xr.get("sfxIds") or []:
            if str(sid) not in sfx_refs:
                missing.append(f"{xid}/{sid}: falta proyección T7 sfx")
    return not missing, sorted(set(missing))


def _route_policy_warnings(piece: dict[str, Any]) -> list[str]:
    policy = piece.get("routePolicy") if isinstance(piece.get("routePolicy"), dict) else {}
    primary = str(policy.get("primaryRoute") or "")
    if primary.lower() in {"vo", "voiceover", "xr", "cine", "sfx", "music", "captions"}:
        return [f"{_pid(piece)}: routePolicy.primaryRoute no debe ser media type ({primary})"]
    return []
...
def validate_r6_project(project: dict[str, Any], libraries: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for index, piece in enumerate(pieces, 1):
        ...
        source_alignment, alignment_warnings, alignment_missing = _source_alignment_report(piece)
        voiceovers, vo_warnings = _voiceover_report(piece)
        pwarnings.extend(alignment_warnings)
        pwarnings.extend(vo_warnings)
        pwarnings.extend(_route_policy_warnings(piece))
        if piece.get("sourceAlignment") is not None:
            missing.extend(alignment_missing)
        projection_ready, projection_missing = _projection_report(piece) if typ in VIDEO else (True, [])
            ...
            if r6_detected:
                ...
                for event in captions:
                    ...
                    if event.get("type") == "caption_group" or event.get("captionPolicyVersion") == "R6":
                        ...
                        elif word_count and not 6 <= word_count <= 17 and not event.get("shortSourceException"):
        ...
        piece_reports.append({
            ...
            "projectionReady": bool(projection_ready),
    ...
    return {
        ...
        "summary": {
            ...
            "projectionReadyCount": sum(1 for row in piece_reports if row["projectionReady"]),
        },
    }
